...
| 012 | STOCKFISH_EVENT_MEMORY | `daytrade/trade_events.py` + runner emission/`--resume` | `[SPEC]`+built 2026-08-09 — IMPLEMENTED → UNIT VERIFIED (21/21, `012_MUTATION_LOG.md`) → **WIRED 2026-08-10** (runner emits every cycle, append-before-apply with a clocked pure pre-validation gate) → **EXERCISED against the real loop** (destroy-state → `--resume` → identical continuation at four kill points incl. the TP2 partial; `test_runner_event_wiring.py`, 15/15 `GATE2_WIRING_MUTATION_LOG.md`). Adversarial-reviewed 2026-08-10: 1 HIGH (C004 dry-run parity) + 2 surviving mutants fixed same day; LOW limitations recorded in the spec. Not yet exercised by a LIVE session (replay-path evidence only); day-later fresh-eyes reread still owed before the week closes. Pending certification. | Gate 2 |
...
| 024 | DISCIPLINE_LAYER | extensions to `daytrade/alpha_operator.py` + `four_books.py`, new `daytrade/gap_log.py`, config contracts `operator/correlated_groups.json` + `limits.json` | `[SPEC]`+built 2026-08-17 — spec written FIRST. Pre-registration (R band + typed predicates, resolver-scored), point-in-time packets (I14), emission gates (session cap, co-drift pause), advisory-only guards, referee-first books + yield.jsonl, sim→live gap tripwire, `--shadow` soak mode. IMPLEMENTED → UNIT VERIFIED (290/290; M17–M26 killed, `024_MUTATION_LOG.md`). Not EXERCISED live; single-seat, certification owed. | after 023 |
| 025 | EXIT_EVALUATOR | `daytrade/exit_evaluator.py` | `[SPEC]`+built 2026-08-17 — pooled state-action evaluator; ran → NO_SUPERSEDE (day-grouped OOF; derived policy lost to the static candidates) | after 023 |
| 026 | COMPETENCE_REPORT | `daytrade/competence_report.py`, `daytrade/oracle_audit.py` | `[SPEC]`+built 2026-08-17 — conditional competence + oracle-audit addendum. Interrupt channel measured at ~zero and RETIRED in code; info-limited realizable leak −0.028 (DRAWER) | after 023 |
| 027 | CARRY_UNWIND_SENTINEL | `daytrade/carry_sentinel.py` (Phase 1 only) | `[SPEC]` amended 2026-08-17 — pinned model + cutoff, NO_EVIDENCE_COHORT hard assertion. First run: 8 unwind + 8 calm windows, evidence cohort 0+0 → zero scored calls permitted; evidence accrues live-forward | blocked on live-forward |
| 028 | SEAL_REGISTRY | `SEALS.json`, `daytrade/seals.py` | `[SPEC]`+built 2026-08-18 — reason-before-look enforced mechanically; drift-watch inside the suite; fault-checked live | done |
| 029 | MECHANISM_LEDGER | `MECHANISMS.json`, `daytrade/mechanisms.py`, `--mechanisms` on `scripts/paper_carry_log.py` | `[SPEC]`+built 2026-08-19 — transfer predictions required, day-blocked permutation null, MDE gate, predictor calibration with partial pooling. M27–M33 killed (`029_MUTATION_LOG.md`); first run re-derived a known negative | done |
...
- **Gate 2: CLOSED 2026-08-10** — the runner emits 012 events
  (append-before-apply honored; destroy/resume proven through the real loop;
